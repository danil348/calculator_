...
void Probability() {
	...
	do {
		n = ChekOnInt();
		if (n < 0) {
			cout << "Количесвто вероятных исходов не может быть меньше нуля! повторите попытку: ";
		}
		else if (n == 0) {
	...
	do {
		...
		if (m > n) {
			...
			cout << "Количество благоприятных сходов не может быть отрицательным! Повторите попытку: ";
		}
	} while (m <= 0 || m > n);
